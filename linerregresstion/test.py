...
def invoke_prediction_lambda(data_to_pass):
...
# Initialize the AWS Bedrock client for Claud

        # Read the response from the invoked Lambda
        response_payload = json.loads(response['Payload'].read().decode('utf-8'))
        print("Response from Target Lambda:", response_payload)

        # Return the response from the target Lambda function
...
def get_answer_from_claude(user_question, suspicious_data, project_times, max_min_data, prediction_data, current_data, response_payload):
    try:
...
def format_data_for_claude(data, data_type):
    ...
    if data_type == "suspicious":
        ...
        return "\n予想データ:\n" + "\n".join([f"• Time: {entry['time']} - Number of People: {entry['num']} - Reason: {entry['reasons']}" for entry in data])
...
def lambda_handler(event, context):
    ...
    # Define the data to pass to the prediction Lambda function
    data_to_pass = {
        ...
        "current_data": current_data,
        "user_question": user_question  # Include the user question or any other necessary data
    }

    # Step 4: Invoke the prediction Lambda
    response_payload = invoke_prediction_lambda(data_to_pass)
    ...
    answer = get_answer_from_claude(user_question, suspicious_data, project_times, max_min, prediction_data, current_data, response_payload)
