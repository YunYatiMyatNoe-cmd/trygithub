    ...
    try:
        if question == "予測":
            context = {
                ...
                'weather_data': "\n".join([f"• Time: {entry['weather_time']} - Temperature: {entry['temperature_2m_celsius']}°C - Humidity: {entry['relative_humidity_2m_percent']}%" for entry in data['weather_times']]),
            ...
            質問: {question}
            【予測に関係する質問】の場合、利用可能なデータ:
            - {context['last_week_data']}（過去のデータ）
            - {context['interval_data']} (3階の食堂の人数データ)
            - {context['weather_data']} (箕面キャンパスの天気予測データ)
            - {context['zone_data']} (3階のエリアに関するデータ)
            - {context['pdf_text']} (箕面キャンパスのスケジュール)
            予測に関係する質問の場合は、上記のデータを参考にして、ユーザーが質問した時間、予測される人数、予測に至った理由を提供してください。
            """
        else:
            context = {
            ...
            【予測に関係しない質問】の場合、利用可能なデータ:
            - 現在の人数データ:{context['current_data']} 
            - 不審者情報:{context['suspicious_data']}
            - 最初と最終データ：{context['project_times']}
            - 最低最高人数：{context['max_min_data']}
            ‐準備情報：{context['pdf_text2']}
            予測に関係しない質問の場合は、「予測に関係しない質問」に該当するデータの中から、最も関連性が高いものを使って質問に関係ある回答のみ正確に答えてください。
            """
