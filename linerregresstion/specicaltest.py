...
def get_answer_from_claude(question, suspicious_data, start_last_times, max_min_data, interval_data, weather_data, zone_data, pdf_text, current_data):
    try:
        # Format suspicious data
        context_current_data = "\n不審者:\n" + "\n".join([f"• Time: {entry['event_time']} - Number of People: {entry['num']}" for entry in current_data])
        ...
        注意点は予測と関係ある質問だけcontext_interval、context_weather、context_zone、context_pdfのデータを使ってください。
...
def lambda_handler(event, context):
    ...
    answer = get_answer_from_claude(user_question, suspicious_data, project_times, max_min, interval_data, weather_times, zone_data, pdf_text, current_data)
