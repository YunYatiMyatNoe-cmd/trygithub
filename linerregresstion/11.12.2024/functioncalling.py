...
def get_answer_from_claude(question, data):
    try:
        ...
        prompt = f"""
        質問: {question}
        下記の指示に従って、質問に関して適切なデータのみを使用して回答してください。

        1. ユーザーの質問が現在のデータについての質問であれば、{context['current_data']}のデータのみを使用してください。
        2. ユーザーの質問が入り帰り時間や人数に関する質問であれば、{context['project_times']}のデータのみを使用してください。
        3. ユーザーの質問が最低または最高人数に関する質問であれば、{context['max_min_data']}のデータのみを使用してください。
        4. ユーザーの質問が不審者に関する質問であれば、{context['suspicious_data']}のデータのみを使用してください。
        5. ユーザーの質問が予測人数に関する質問であれば、{context['last_week_data']}、{context['project_times']}、{context['interval_data']}、{context['weather_data']}、{context['pdf_text']}のその五つのデータのみを使用してください。
        
        このガイドラインに従い、質問に答えてください。
        """
