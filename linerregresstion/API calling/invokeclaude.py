...
# Generate the prompt for Bedrock with function calling capabilities
def get_answer_from_claude(question, data):
    try:
        # Claude expects a structured approach with function calls
        context = {
            'current_data': data['current_data'],
            'last_week_data': data['last_week_data'],
            'suspicious_data': data['suspicious_data'],
            'project_times': data['project_times'],
            'max_min_data': data['max_min'],
            'interval_data': data['interval_data'],
            'weather_data': data['weather_times'],
        ...
        ・last_week_data: {context['last_week_data']}
        ・suspicious_data: {context['suspicious_data']}
        ・project_times: {context['project_times']}
        ・max_min_data: {context['max_min_data']}
        ・interval_data: {context['interval_data']}
        ・weather_data: {context['weather_data']}
        ・zone_data: {context['zone_data']}
        ・pdf_text: {context['pdf_text']}
        ・pdf_text2: {context['pdf_text2']}
        """
            ...
            "functions": [
                {
                    ...
                    "parameters": {"function_name": "get_current_time_data"}
                }
            ],
            "max_tokens": 2048,
