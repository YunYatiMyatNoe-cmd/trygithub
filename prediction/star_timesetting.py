...
context_last_week_data is the third floor data last 1 week.
context_interval of num is the num of people is third floor canteen data 
and context_weather is the minohc campus of prediction weather and context_zone is the third floor area,
context_pdf is the schedule of minoch campus I want to know prediction data formatted_time_japan base these three data because I want to prepare food.
...
def lambda_handler(event, context):
    ...
    formatted_time_japan = one_hour_later.strftime("%Y-%m-%d+%H:%M:%S")

    print("formatted_next_hour_time_japan" + formatted_time_japan)
    
    answer = get_answer_from_claude(f"Predict the number of people at {formatted_time_japan}", interval_data, weather_times, zone_data, pdf_text, last_week_data)
    ...
    try:
        ...
        if isinstance(answer, list) and len(answer) > 0:
            ...
            # Check if the response contains a valid JSON string inside the code block
            if response_text:
                # Remove the markdown code block delimiters (```json ... ```), if present
                ...
                try:
                    ...
                    if prediction_time and predicted_num is not None and reasons:
                        ...
                        # Insert the prediction data into Supabase
