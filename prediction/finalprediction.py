...
def lambda_handler(event, context):
    ...
    try:
        ...
        # The response is a list with one dictionary, so we need to access the first item in the list
        if isinstance(answer, list) and len(answer) > 0:
            response_text = answer[0].get('text', '')  # Extracting the text from the first item of the list
            ...
            if response_text:
                try:
                    ...
                    reasons_str = " ".join(prediction_data.get('reasons', []))
                    # Now prepare to save the data to Supabase
                    if prediction_time and predicted_num is not None:
                        ...
                        # Ensure we're inserting a dictionary with the correct format
